...
val client_no_received_clientrecoveryresponses: clien_state -> int
...
val log: replica_state -> (StateMachine.operation * int * int) list
...
val log_recoveryresponse: replica_state -> index -> int -> ((StateMachine.operation * index * index) list * index * index) option -> index
...
val update_client_table: ?(res:StateMachine.result) -> replica_state -> index -> index -> replica_state
