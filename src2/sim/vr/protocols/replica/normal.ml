...
let start_replica state = 
  if is_primary state then
    (state, [Timeout(ReplicaTimeout(HeartbeatTimeout(valid_timeout state, op_no state), int_of_index (replica_no state)))])
  else
  ...
  if (is_primary state && (status state) = Normal) then
    ...
    if (s < cte_s) then
      ...
      | Some(res) -> (* return result for most recent operation *) 
        (state, [Communication(Unicast(ClientMessage(Reply(view_no state, s, res)), int_of_index c))])
    else 
      ...
      let prepare_timeout = ReplicaTimeout(PrepareTimeout(valid_timeout state, op_no), int_of_index (replica_no state)) in
      let heartbeat_timeout = ReplicaTimeout(HeartbeatTimeout(valid_timeout state, op_no), int_of_index (replica_no state)) in
    ...
    if (op_no state) >= n then
      ...
      let events = 
        if op_no > (op_no state) then 
          [Communication(Unicast(ReplicaMessage(PrepareOk(view_no state, op_no, replica_no state)), int_of_index primary_no))]
        else 
          [Timeout(ReplicaTimeout(StateTransferTimeout(valid_timeout state, op_no state), int_of_index (replica_no state)))] in
