...
let on_heartbeat_timeout state v n = 
  if (valid_timeout state) = v && (op_no state) = n then
    ...
    (state, [Communication(Broadcast(ReplicaMessage(Commit(view_no state, commit_no state)))); 
             Timeout(ReplicaTimeout(HeartbeatTimeout(valid_timeout state, op_no state), int_of_index (replica_no state)))])
  else
    (state, [])
...
let on_startviewchange_timeout state v = 
  if (valid_timeout state) = v then
    ...
    (state, [Communication(multicast); Timeout(ReplicaTimeout(StartViewChangeTimeout(valid_timeout state), int_of_index (replica_no state)))])
  else 
    (state, [])
...
let on_recovery_timeout state v = 
  if state.valid_timeout = v then
    ...
    (state, [Communication(multicast); Timeout(ReplicaTimeout(RecoveryTimeout(valid_timeout state), int_of_index (replica_no state)))])
  else 
    (state, [])
